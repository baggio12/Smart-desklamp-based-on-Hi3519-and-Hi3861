...
#define CN_IOT_SERVER    "tcp://8eed3fb5ab.st1.iotda-device.cn-north-4.myhuaweicloud.com:1883"
...
void MqttProcess(MQTTClient client, char *clientID, char *userPwd, MQTTClient_connectOptions connOpts, int subQos[])
{
    ...
    printf("Connect Iot Server success\r\n");
    ...
    while (MQTTClient_isConnected(client)) {
        ...
        int ret = ProcessQueueMsg(client);
...
int IoTMain(void)
{
    hi_u32 ret;
    hi_task_attr attr = {0};
    printf("Create the Main Entry start\r\n");
    ...
    if (ret != HI_ERR_SUCCESS) {
        printf("Create the Main Entry Failed\r\n");
    }
    printf("Create the Main Entry success\r\n");
